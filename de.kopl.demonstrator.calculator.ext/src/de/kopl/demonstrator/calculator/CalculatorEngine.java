...
public class CalculatorEngine {
	...
	public void evaluateCommand(final Commands command) {
		...
		case MEMORY_ADD: // Add to Memory
			...
			updateMemoryString(tempMemoryString);
			clearDisplay = true;
			calculator.appendHistoryEntry("Memory + " + displayString + " = " + tempMemoryString + " > Memory\n");
			break;
		...
		case INVERSE: // Inverse
			tempString = doCalc(displayString, "", command);
			clearDisplay = true;
			updateCalculatorDisplay(tempString);
			calculator.appendHistoryEntry(" " + command + " \n = " + tempString);
			break;
		...
		case CHANGE_SIGN: // Change Sign
			if (tempString.startsWith("-")) {
				...
				calculator.replaceLatestHistoryEntry("-" + tempString, tempString);
			} else {
				tempString = command + tempString;
				calculator.replaceLatestHistoryEntry(trimLeadingMinusSign(tempString), tempString);
			}
			clearDisplay = false;
			updateCalculatorDisplay(tempString);
			break;
		...
		case ZERO: // Don't want 00 to be entered.
			if (!tempString.equals("0") && tempString.length() < 29) {
				tempString = tempString + command;
				calculator.appendHistoryEntry(command.toString());
			}
			updateCalculatorDisplay(tempString);
			clearDisplay = false;
			break;
	...
	private String doCalc(final String valAString, final String valBString, final Commands command) {
		...
		if (command != Commands.SQUARE_ROOT && command != Commands.INVERSE && command != Commands.SINUS
				&& command != Commands.COSINUS && command != Commands.TANGENS) {
		...
		case SINUS:
			valAnswer = Math.sin(valA);
			break;
	
		case COSINUS: 
			valAnswer = Math.cos(valA);
			break;

		case TANGENS:
			valAnswer = Math.tan(valA);
			break;
